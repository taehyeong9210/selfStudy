...
// 홍 박사님 연구실의 폰켓몬은 종류에 따라 번호를 붙여 구분합니다. 따라서 같은 종류의
// 폰켓몬은 같은 번호를 가지고 있습니다. 예를 들어 연구실에 총 4마리의 폰켓몬이 있고,
...
// 쉽게 이야기해서 중이 허용되지 않는 배열을 활용해라
function solution(nums) {
  const pick = new Set(nums);
  return pick.size <= nums.length / 2 ? pick.size : nums.length / 2; // new Set 객체는 length가 아닌 size 프로퍼티를 사용
}
